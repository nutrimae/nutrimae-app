            ...
            return (
              <div key={category.key} className={`rounded-2xl p-4 ${style.card}`}>
                <div className="flex items-center gap-2">
                  ...
                  {style.badge && (
                    <span className={`ml-auto rounded-full px-2 py-0.5 text-[11px] font-bold ${style.badge}`}>
                      SOS
                    </span>
                  )}
                </div>
                <p className="mt-1 text-xs text-brown-700/86">{category.description}</p>
        ...
        <div className="-mx-4 flex snap-x snap-mandatory gap-3 overflow-x-auto px-4 pb-2">
          {LAXATIVE_RECIPES.map((recipe) => (
            ...
            >
              ...
              <Chip color="sage" className="w-fit"><Clock className="h-3 w-3" strokeWidth={2} />{recipe.prepMinutes} min</Chip>
              <ul className="flex flex-col gap-1 text-xs text-brown-700/90">
              ...
              <p className="mt-1 text-xs leading-relaxed text-brown-700/86">{recipe.steps}</p>
            </article>
          ))}
        </div>
      </section>
