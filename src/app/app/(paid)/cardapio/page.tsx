...
export default function CardapioPage() {
  ...
  const [activeTab, setActiveTab] = useState<"refeicoes" | "lancheira">("refeicoes");
  ...
  return (
    <main className="mx-auto flex w-full max-w-sm flex-col gap-5 px-4 py-6">
      {/* 24+ months view switcher */}
      {isPost24Months && (
        <div className="flex rounded-2xl bg-sage-100/70 p-1">
          <button
            type="button"
          ...
          <button
            type="button"
            onClick={() => setActiveTab("lancheira")}
            className={`flex-1 rounded-xl py-2 text-xs font-bold transition-all ${
              activeTab === "lancheira"
      ...
      {isPost24Months && activeTab === "lancheira" ? (
        <>
          <LunchboxPlanner babyId={activeBaby.id} babyName={activeBaby.name.split(" ")[0]} />
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      {!isOpen && (
                        <p className="mt-1 text-sm text-brown-700/90">{suggestion.description}</p>
                      )}
                    </div>
                    <ChevronDown
