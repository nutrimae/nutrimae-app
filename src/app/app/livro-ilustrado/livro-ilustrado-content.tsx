    ...
    <main className="mx-auto flex w-full max-w-sm flex-col gap-5 px-4 py-5 pb-28">
      ...
      {!state.eligible && (
        <section className="glass-card rounded-3xl p-5">
          <div className="flex items-center gap-3">
            ...
            <div><h2 className="font-heading font-bold text-brown-800">A historia esta ganhando forma</h2><p className="text-xs text-brown-700/86">O convite aparece quando houver material suficiente.</p></div>
          </div>
      ...
      {state.eligible && !state.hasAccess && (
        <section className="rounded-3xl border border-primary-100 bg-white p-5 shadow-[0_10px_35px_rgba(255,107,157,0.10)]">
          ...
          <div className="mt-2 flex items-end justify-between"><div><p className="text-sm text-brown-700/86 line-through">R$ 149</p><p className="font-heading text-3xl font-bold text-brown-800">R$ 119</p></div><Chip color="sage">pagamento unico</Chip></div>
      ...
      {state.eligible && state.hasAccess && !ready && (
        <section className="glass-card rounded-3xl p-5">
          ...
          <p className="mt-1 text-sm text-brown-700/88">A foto e opcional. Sem ela, criamos um personagem ilustrado generico.</p>
          <button type="button" onClick={() => fileRef.current?.click()} className="mt-4 flex min-h-24 w-full items-center gap-3 rounded-2xl border-2 border-dashed border-primary-200 bg-primary-50/50 px-4 text-left">
            <ImagePlus className="h-7 w-7 text-primary-500" /><span><strong className="block text-brown-800">{photo ? photo.name : "Escolher uma foto"}</strong><span className="text-xs text-brown-700/84">JPG, PNG ou WebP, ate 5 MB</span></span>
          </button>
          ...
          <div className="mt-3 flex items-center gap-2 text-xs text-brown-700/86"><LockKeyhole className="h-4 w-4 text-sage-600" />Arquivos privados e nunca publicados automaticamente.</div>
      ...
      {ready && state.book && (
        <section className="rounded-3xl bg-gradient-to-br from-sage-50 to-white p-5 text-center shadow-[0_12px_40px_rgba(16,185,129,0.12)]">
          ...
          <p className="mt-1 text-sm text-brown-700/88">As paginas passaram pela revisao automatica e estao prontas para guardar.</p>
      ...
      <p className="px-3 text-center text-[11px] leading-relaxed text-brown-700/80">Esta e uma lembranca afetiva criada a partir dos registros informados pela responsavel. Nao e orientacao medica ou nutricional.</p>
    </main>
  );
}
