 ...
 */
export function VipUpgradeScreen({ locale = "pt-BR" }: { locale?: Locale }) {
    ...
    <main className="mx-auto flex w-full max-w-sm flex-col gap-6 px-4 py-10">
      <div className="flex flex-col items-center text-center">
        ...
        <div className="mb-2 flex items-center gap-1.5 rounded-full bg-gradient-to-r from-amber-100 to-amber-50 px-3 py-1">
          ...
          <span className="text-xs font-semibold text-amber-700">{es ? "Área VIP" : "Área VIP"}</span>
        </div>
        <h1 className="font-heading text-2xl font-bold text-brown-800">
          {es ? "Los dos apoyos extra para los momentos más difíciles" : "Os dois apoios extras para os momentos mais difíceis"}
        </h1>
        <p className="mt-2 text-sm text-brown-700/90">
          {es ? "Desbloquea el Área VIP y accede a los dos módulos completos." : "Desbloqueie a Área VIP e tenha os dois módulos completos."}
        </p>
      </div>
      ...
      <div className="glass-card rounded-3xl p-6">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-0.5 text-xs text-brown-700/86">
              {es ? "Playlist de audio y rastreador para las madrugadas del destete" : "Playlist de áudio e rastreador para as madrugadas de desmame"}
            </p>
          </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-0.5 text-xs text-brown-700/86">
              {es ? "Semáforo de la caquita y recetas para el estreñimiento" : "Semáforo do cocô e receitas para o intestino preso"}
            </p>
          </div>
        ...
        <div className="mt-1 flex items-center justify-between">
          <span className="font-heading text-base font-bold text-brown-800">{es ? "Área VIP completa por" : "Área VIP completa por"}</span>
          <span className="font-heading text-2xl font-bold text-amber-600">{formatPrice(totalPromo)}</span>
        </div>
        <p className="mt-1 text-right text-xs text-brown-700/82">{es ? "pago único, acceso de por vida" : "pagamento único, acesso vitalício"}</p>

        <div className="mt-4 flex items-center gap-2 text-xs text-sage-600">
          <Check className="h-3.5 w-3.5 shrink-0" strokeWidth={2.5} />
          {es ? "También puedes comprar cada módulo por separado" : "Também pode comprar cada módulo separadamente"}
        </div>
      </div>
