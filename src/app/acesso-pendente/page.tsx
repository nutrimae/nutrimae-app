  ...
  return (
    <main className="flex min-h-dvh items-center justify-center bg-gradient-to-b from-primary-50 via-cream to-sage-50 px-6 py-10">
      <section className="w-full max-w-sm rounded-3xl bg-white p-6 text-center shadow-strong">
        ...
        <h1 className="mt-4 text-2xl font-bold text-brown-800">{es ? "Acceso a la espera de liberación" : "Acesso aguardando liberação"}</h1>
        <p className="mt-3 text-sm leading-relaxed text-brown-700/65">
          {es
            ? "El web app es exclusivo para clientas. Si acabas de comprar, la confirmación puede tardar unos instantes."
        ...
        <button type="button" onClick={checkAgain} disabled={checking} className="mt-6 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-primary-500 to-primary-600 font-semibold text-white disabled:opacity-60">
          ...
          {checking ? (es ? "Verificando..." : "Verificando...") : es ? "Verificar mi compra" : "Verificar minha compra"}
        </button>
        <Link href="/oferta" className="mt-3 flex min-h-12 items-center justify-center rounded-2xl border border-primary-100 text-sm font-semibold text-primary-600">{es ? "Todavía no compré" : "Ainda não comprei"}</Link>
        <button type="button" onClick={signOut} className="mt-3 flex min-h-11 w-full items-center justify-center gap-2 text-sm text-brown-700/50"><LogOut className="h-4 w-4" />{es ? "Usar otro correo" : "Usar outro e-mail"}</button>
        <p className="mt-5 flex items-center justify-center gap-1.5 text-[11px] text-brown-700/40"><ShieldCheck className="h-3.5 w-3.5" />{es ? "Compra validada en el servidor" : "Compra validada no servidor"}</p>
      </section>
    </main>
  );
}
